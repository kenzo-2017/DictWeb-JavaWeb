	...
	public List<Diary> queryDiary(String sql) {
		...
		try { // 捕捉异常
			while (rs.next()) {
				...
				diary.setId(rs.getInt(1)); // 获取并设值ID
				diary.setTitle(rs.getString(2)); // 获取并设值日记标题
				diary.setAddress(rs.getString(3)); // 获取并设值图片地址
				Date date;
				date = rs.getDate(4);
				diary.setWriteTime(date); // 设值写日记的时间
//				try {
...
//				}
				diary.setUserid(rs.getInt(5)); // 获取并设值用户ID
				diary.setUsername(rs.getString(6)); // 获取并设值用户名
