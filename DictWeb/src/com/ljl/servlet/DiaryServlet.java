...
import com.ljl.dao.DiaryDao;
import com.ljl.model.Diary;
import com.ljl.tools.MyPagination;
...
public class DiaryServlet extends HttpServlet {
	 ...
	 * @throws IOException
	 */
	public void listMyDiary(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		...
		if (strPage == null) {
			int userId = Integer.parseInt(session.getAttribute("uid").toString()); // 获取用户ID号
			String sql = "SELECT d.*,u.username FROM tb_diary d INNER JOIN tb_user u ON u.idd.userid WHERE " + 
					"d.userid=" + userId + " ORDER BY d.writeTime DESC"; // 应用内连接查询日记信息
		...
		request.getRequestDispatcher("listAllDiary.jsp").forward(request, response); // 转发页面
	}
	...
	public void preview(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < content.length; i++) {
			if (content[i].equals(null) || content[i].equals("") || content[i].equals("请在此输入文字")) {
				content[i] = "没啥可说的"; // 为没有输入内容的项目设置默认值
			}
		}
		...
		session.setAttribute("diary", content); // 保存日记的内容
		request.getRequestDispatcher("preview.jsp").forward(request, response); // 转发页面
	}
	...
	public void save(HttpServletRequest request, HttpServletResponse response)
		...
		url = request.getSession().getServletContext().getRealPath("/"); // 获取请求的实际地址
		...
		ImageIO.write(image, "PNG", new File(scaleImgUrl)); // 保存缩略图文件
		/*************************将填写的日记保存到数据库中**************************/
	...
	public void listAllDiary(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String strPage = (String) request.getParameter("page"); // 获取当前页码
		int page = 1;
		List<Diary> list = null;
		if (strPage == null) { // 当页面初始运行
			String sql = "SELECT d.*,u.username FROM tb_diary d INNER JOIN tb_user u ON u.id=d.userid ORDER BY " + 
				"d.writeTime DESC LIMIT 50";
			pagination = new MyPagination();
			list = dao.queryDiary(sql); // 获取日记内容
			int pageSize = 4; // 指定每页显示的记录数
			list = pagination.getInitPage(list, page, pageSize); // 初始化分页信息
		...
		request.getRequestDispatcher("listAllDiary.jsp").forward(request, response); // 请求转发
	}
}
