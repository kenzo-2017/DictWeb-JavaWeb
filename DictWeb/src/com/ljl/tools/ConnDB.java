package com.ljl.tools; // 将该类保存到com.ljl.tools包中
...
public class ConnDB {
	...
	private static String dbUrl = "jdbc:mysql://127.0.0.1:3306/db_9griddaary?" + 
	...
	public void close() {
		try { // 捕捉异常
			if (rs != null) { // 当ResultSet对象的实例不为空时
				rs.close(); // 关闭ResultSet对象
			}
			if (stmt != null) { // 当Statement对象的实例不为空时
				stmt.close(); // 关闭Statement对象
			}
			if (conn != null) { // 当Connection对象的实例不为空时
