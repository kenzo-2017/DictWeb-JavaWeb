...
<jsp:useBean id="pagination" class="com.ljl.tools.MyPagination" scope="session" />
    ...
    <div id="box">
    	...
    	<c:if test="${!empty requestScope.diaryList}">
    		<c:forEach items="${requestScope.diaryList}" var="diaryList" varStatus="id">
    			<script type="text/javascript">
    				...
    				function rotate${id.count}() {
	    				...
	    				var rotate = function(canvas, img, rot) {
	    					...
	    					context.save();
	    					// 改变中心点
	    					if (rotation <= Math.PI / 2) { // 旋转角度小于等于90度时
	    						context.translate(s * h, 0);
	    					} else if (rotation <= Math.PI) { // 旋转角度小于等于180度时
    			...
    			<div style="border-bottom-color:#CBCBCB;padding:5px;border-bottom-style:dashed;border-bottom-width:1px;
    				margin:10px 20px;color:#0F6548;">
    				<font color="#CE6A1F" style="font-weight:bold;font-size:14px;">
    					${diaryList.username}
    				</font>
    				&nbsp;&nbsp;发表九宫格日记：
    				<b>${diaryList.title}</b>
    			</div>
    			<div style="margin:10px 10px 0px 10px;background-color:#DDDDDD solid;width:60px;background-color:#EEEEEE;">
