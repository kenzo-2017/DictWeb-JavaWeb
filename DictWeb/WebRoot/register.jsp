...
<script language="javascript" src="JS/ljlFunction.js"></script>
...
function checkUser(str) {
	if (str == "") { // 当用户名为空时
		...
		flag_user = false;
	} else if (!myCheckUser(str)) {
		document.getElementById("div_user").innerHTML = "您输入的用户名不合法！"; // 设置提示文字
		document.getElementById("tr_user").style.display = 'block'; // 显示提示信息
		flag_user = false;
	} else {
		var loader = new net.AjaxRequest("UserServlet?action=checkUser&username=" + str + 
			"&nocache=" + new Date().getTime(), deal, onerror, "GET");
	}
}
function deal() {
	result = this.req.responseText; // 获取返回的检测结果
	result = result.replace(/\s/g, ""); // 去掉Unicode空白符
	if (result == "1") { // 当用户名没有被注册
		document.getElementById("div_user").innerHTML = ""; // 清空提示文本
...
function checkPwd(str) {
	if (str == "") { // 当密码为空时
		...
		flag_pwd = false;
	} else if (!myCheckPwd(str)) { // 当密码不合法时
...
function checkRepwd(str) {
	if (str == "") { // 当确认密码为空时
		...
		document.getElementById("tr_pwd").style.display = 'block'; // 显示提示信息
		flag_repwd = false;
	} else if (regForm.pwd.value != str) { // 当密码不合法时
...
function checkQuestion(str_q, str_a) {
	if (str_q != "" && str_a == "") { // 当密码提示问题不为空，而提示问题答案为空时
		...
		document.getElementById("div_answer").innerHTML = "请输入密码提示问题！"; // 设置提示信息
		document.getElementById("tr_answer").style.display = 'block'; // 显示提示信息
		flag_answer = false;
	} else {
...
function save() {
	if (regForm.user.value == "") { // 当用户名为空时
		alert("请输入用户名！");
		regForm.user.focus();
		return;
	}
	if (regForm.pwd.value == "") { // 当密码为空时
		alert("请输入密码！");
		regForm.pwd.focus();
		return;
	}
	if (regForm.repwd.value == "") { // 当没有输入确认密码时
		alert("请确认密码！");
		regForm.repwd.focus();
		return;
	}
	if (regForm.email.value == "") { // 当E-mail地址为空时
		alert("请输入E-mail地址！");
		regForm.email.focus();
		return;
	}
	...
	if (flag_user && flag_pwd && flag_repwd && flag_email && flag_question && flag_answer) {
		var param = "user=" + regForm.user.value + "&pwd=" + regForm.pwd.value + "&email=" + 
			regForm.email.value + "&question=" + regForm.question.value + "&answer=" + 
			regForm.answer.value + "&city=" + regForm.city.value; // 组合参数
...
function deal_save() {
	...
	form_reset(regForm); // 重置表单
	myClose("register"); // 隐藏用户注册页面
}
function getProvince() {
	var loader = new net.AjaxRequest("UserServlet?action=getProvince&nocache=" + new Date().getTime(), 
...
function deal_getProvince() {
	...
	for (i = 0; i < provinceArr.length; i++) { // 设置下拉列表框的列表项
		document.getElementByAId("province").options[i] = new Option(provinceArr[i], provinceArr[i]);
	}
	if (provinceArr[0] != "") {
...
function getCity(selProvince) {
	var loader = new net.AjaxRequest("UserServlet?action=getCity&parProvince=" + selProvince + "&nocache=" + 
			...
			<tr>
				<td height="408" align="center" valign="top">
					<table width="100%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							<td height="50" style="color:#1B7F5D;font-size:14px;">
								<b>清爽夏日九宫格日记网——用户注册</b>
							</td>
						</tr>
					</table>
					<table width="94%" height="331" border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
						<tr>
							<td height="310" valign="top" bgcolor="#FFFFFF">
								<table border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFEF9">
									<tr id="tr_user" style="display:none">
										<td height="40" colspan="2" align="center">
											<div id="div_user" style="border:#FF6600 1px solid;color:#FF0000;width:90%;height:29px;padding-top:8px;">
											</div>
										</td>
									</tr>
									<tr>
									...
									<tr id="tr_pwd" style="display:none">
										<td height="40" colspan="2" align="center">
											<div id="div_pwd" style="border:#FF6600 1px solid;color:#FF0000;width:90%;height:29%;padding-top:8px;
									...
									<tr id="tr_email" style="display:none">
										<td height="40" colspan="2" align="center">
											<div id="div_email" style="border:#FF6600 1px solid;color:#FF0000;width:90%;height:29px;padding-top:8px;
												background-image:url(images/div_bg.jpg)">
											</div>
										</td>
									</tr>
									<tr>
										...
										<td height="40" align="left">
											<input name="email" type="text" size="35" onBlur="checkEmail(this.value)" />
									...
									<tr id="tr_question" style="display:none">
										<td height="40" colspan="2" align="center">
											<div id="div_question" style="border:#FF6600 1px solid;color:#FF0000;width:90%;height:29px;padding-top:8px;
												background-image:url(images/div_bg.jpg)">
											</div>
										</td>
									</tr>
									<tr>
										...
										<td height="40" align="left">
											<input name="question" type="text" id="question" size="35" onBlur="checkQuestion(this.value, this.form.asnwer.value)" />
											如：我的工作单位
										</td>
									</tr>
									<tr id="tr_answer" style="display:none">
										<td height="40" colspan="2" align="center">
											<div id="div_answer" style="border:#FF6600 1px solid;color:#FF0000;width:90%;height:29px;padding-top:8px;
												background-image:url(images/div_bg.jpg)">
											</div>
										</td>
									</tr>
									<tr>
										...
										<td height="40" align="left">
											<input name="answer" type="text" id="answer" size="35" onBlur="checkQuestion(this.form.question.value,this.value)" />
											如：明日科技
										</td>
									</tr>
									<tr>
										...
										<td height="40" align="center">
											<input name="btn_submit" type="button" class="btn_grey" value="提交" onClick="save()" />
											&nbsp;
